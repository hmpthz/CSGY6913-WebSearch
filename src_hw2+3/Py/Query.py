...
def init_dll() -> ctypes.CDLL:
    ...
    cdll.get_TermsInfo.argtypes = [ctypes.py_object, ctypes.c_void_p]
    cdll.get_TermsInfo.restype = ctypes.c_void_p
...
def generate_snippet(terms: List[str], offset: int) -> str:
    ...
    for term in terms:
        ...
        res = re.search(f'(^|[\W_])({term})([\W_]|$)', text)
...
def print_result(terms: List[str], query_res, terms_info):
    terms_index_len = []
    for i, term in enumerate(terms):
        n_docs = terms_info.terms_n_docs[i]
    ...
    for i in range(query_res.n):
        ...
        terms_freqs = []
        for j in range(terms_info.arr[i].n):
            term_i = terms_info.arr[i].idxs[j]
            freq = terms_info.arr[i].freqs[j]
...
def execute_query(query: str):
    ...
    terms_b = [term.encode('utf-8') for term in terms]
    terms_info_p = cdll.get_TermsInfo(terms_b, query_res_p)
    terms_info = ctypes.cast(terms_info_p, ctypes.POINTER(TermsInfoArray)).contents

    print_result(terms, query_res, terms_info)
    dt = time.time() - dt
    print('[Query time] {:.3f} seconds\n'.format(dt))
    
    cdll.delete_QueryProcessor(root_processor)
    cdll.delete_QueryResult(query_res_p, terms_info_p)
...
if __name__ == '__main__':
    ...
    global tk, pattern_no_url
    # tokenizer and regex for processing query string
